...
def login():
    ...
    if request.method == 'POST':
        ...
        if successResponse(response):
            ...
            response = rest.request("/api/users/<username>",
...
def register(source=None):
    ...
    if request.method == 'POST':
        ...
        response = rest.request("/api/users/<username>/register",
                                {'username': username, 'password': password, 'personName': personName, 'email': email})
...
def buildingDetail(buildingName=None):
    ...
    response = rest.request("/api/users/<username>/buildings/<buildingName>",
...
@gui.route('/buildings/<buildingName>/rooms/<roomName>/graphicalViewSummary/json/', methods=['GET', 'POST'])
@gui.route('/buildings/<buildingName>/rooms/<roomName>/graphicalViewSummary/json', methods=['GET', 'POST'])
def getGraphicalViewSummaryJson(buildingName=None, roomName=None):
    if not loggedIn():    return redirect(url_for('gui.login'))

    username = session["username"]

    filePath = "tmp/summary/json/" + username + "_" + buildingName + "_" + roomName + ".json"
    if not os.path.exists(filePath): return "[]"
...
@gui.route('/buildings/<buildingName>/rooms/<roomName>/graphicalViewSummary/', methods=['GET', 'POST'])
@gui.route('/buildings/<buildingName>/rooms/<roomName>/graphicalViewSummary', methods=['GET', 'POST'])
def roomGraphicalViewSummmary(buildingName=None, roomName=None):
    if not loggedIn():    return redirect(url_for('gui.login'))

    ganttJsonLink = url_for("gui.getGraphicalViewSummaryJson", buildingName=buildingName, roomName=roomName)
    ...
    thisRoomrules = roomRules[roomName]

    ganttBarColors = ['ganttRed', 'ganttGreen', 'ganttBlue', 'ganttOrange']
    currentBarColor = 0

    for rule in thisRoomrules:
        ...
        if  hours != []:
            ...
            unixTsTo = EPOCH(today + " " + hours[1].replace(".",":"))
        else :
            unixTsFrom = EPOCH(today + " " + "00:00")
            unixTsTo = EPOCH(today + " " + "23:59")

            if ganttView != [] :
                for retreivedItem in ganttView :
                    if retreivedItem["values"][0]["label"] == rule["consequent"] :
                        oldDesc = retreivedItem["values"][0]["desc"]
                        ganttView.remove(retreivedItem)
                        description += "<br>" + oldDesc

        unixTsFrom = str(int(unixTsFrom))
        unixTsTo = str(int(unixTsTo))

        item["values"].append(
            {"desc": description, "from": "/Date(" + unixTsFrom + ")/", "to": "/Date(" + unixTsTo + ")/",
             "label": rule["consequent"], "customClass": ganttBarColors[currentBarColor]})
    ...
    if not os.path.exists("tmp/summary/json/"): os.makedirs("tmp/summary/json/")

    out_file = open("tmp/summary/json/" + username + "_" + buildingName + "_" + roomName + ".json", "w")
    out_file.write(json.dumps(ganttView, separators=(',', ':')))
    out_file.close()
    return render_template('summary.html', ganttJsonLink=ganttJsonLink)
    ...
    if request.method == 'POST':
        ...
        for target in response["simulation"].keys():
            ...
            for bar in response["simulation"][target]:
                ...
                item["values"].append(
                    {"desc": bar["ruleText"], "from": "/Date(" + unixTsFrom + ")/", "to": "/Date(" + unixTsTo + ")/",
                     "label": bar["status"], "customClass": ganttBarColors[currentBarColor]})
                currentBarColor += 1
                if currentBarColor == len(ganttBarColors): currentBarColor = 0
...
@gui.route('/buildings/<buildingName>/rooms/<roomName>/game/', methods=['GET', 'POST'])
@gui.route('/buildings/<buildingName>/rooms/<roomName>/game', methods=['GET', 'POST'])
...
def rooms(buildingName=None):
    ...
    for authorUuid in authorList.keys():
        response = rest.request("/api/users/uuid/<uuid>", {'sessionKey': session["sessionKey"], 'uuid': authorUuid,
                                                           'userUuid': session["userUuid"]})
    ...
    for groupId in groupList.keys():
        response = rest.request("/api/users/<username>/buildings/<buildingName>/groups/<groupId>",
                                {'username': session["username"], 'buildingName': buildingName, 'groupId': groupId,
                                 'sessionKey': session["sessionKey"], 'userUuid': session["userUuid"]})
    ...
    return render_template('rooms.html', roomList=roomList, alreadyLoggedIn = alreadyLoggedIn,categories = categories,mapFileName = mapFileName)

@gui.route('/buildings/<buildingName>/rooms/<roomName>/', methods=['GET', 'POST'])
@gui.route('/buildings/<buildingName>/rooms/<roomName>', methods=['GET', 'POST'])
def room(buildingName=None, roomName=None):
    if not loggedIn():    return redirect(url_for('gui.login'))
    ...
    for rule in roomRules[roomName]:
        ...
        if hours == []:
            ...
            hourFrom = float(hours[0])
            hourTo = float(hours[1])
            displayGantt = True

            if 0.0 <= hourFrom <= 6.0 and 0.0 <= hourTo <= 6.0 :
                periodMap[rule["antecedent"]]= "NIGHT"
            elif 6.0 <= hourFrom <= 12.0 and 6.0 <= hourTo <= 12.0 :
                periodMap[rule["antecedent"]]= "MORNING"
            elif 12.0 <= hourFrom <= 18.0 and 12.0 <= hourTo <= 18.0 :
                periodMap[rule["antecedent"]]= "AFTERNOON"
            elif 18.0 <= hourFrom <= 24.0 and 18.0 <= hourTo <= 24.0 :
                periodMap[rule["antecedent"]]= "EVENING"
            else:
                periodMap[rule["antecedent"]]= "ALLPERIOD"
...
def groups(buildingName=None):
    ...
    return render_template('groups.html', groupList=groupList, roomsGroup=roomsGroup, rulesGroup=rulesGroup,
...
def groupDetail(buildingName=None, groupId=None):
    ...
    response = rest.request("/api/users/<username>/buildings/<buildingName>/groups/<groupId>",
                            {'groupId': groupId, 'username': session["username"], 'buildingName': buildingName,
                             'sessionKey': session["sessionKey"], 'userUuid': session["userUuid"]})
...
def addGroup(buildingName=None):
    ...
    if request.method == 'GET':
        ...
        crossRoomsValidation = request.form[
            'crossRoomsValidation'] if 'crossRoomsValidation' in request.form.keys() else False
        ...
        for room in roomList:
            ...
            if ("room_" + room["roomName"]) in request.form.keys():
                response = rest.request(
                    "/api/users/<username>/buildings/<buildingName>/groups/<groupId>/rooms/<roomName>/add", {
                        'username': session["username"],
                        'buildingName': buildingName,
                        'groupId': groupId,
    ...
    if request.method == 'POST':
        ...
        if successResponse(response):
            ...
            if response["request-errorName"] == "RuleValidationError":
                return redirect(
                    url_for('gui.addRuleToRoom', buildingName=buildingName, roomName=roomName, ruleBody=ruleBody,
                            priority=priority, conflictFound=True))
        ...
        return render_template('ruleForm.html', insertionForRoom=True, availableTriggers=availableTriggers,
                               availableActions=availableActions, ruleBody=ruleBody, priority=priority,
                               conflictingRuleList=conflictingRuleList, conflictFound=conflictFound, roomName=roomName,
                               buildingName=buildingName)
...
def addRuleToGroup(buildingName=None, groupId=None):
    ...
    if request.method == 'POST':
        ...
        return render_template('ruleForm.html', insertionForGroup=True, availableTriggers=availableTriggers,
...
def editRoomRule(buildingName=None, roomName=None, ruleId=None, groupId=None):
    ...
    if request.method == 'POST':
        ...
        return render_template('ruleForm.html', rule=rule, insertionForRoom=True, availableTriggers=availableTriggers,
                               availableActions=availableActions, conflictingRuleList=conflictingRuleList,
                               conflictFound=conflictFound, roomName=roomName, buildingName=buildingName)
...
def deleteRoomRule(buildingName=None, roomName=None, ruleId=None):
    ...
    #return redirect(url_for('gui.rooms', buildingName = buildingName))
    return redirect('/buildings/' + buildingName + '/rooms/' + roomName)
...
def editGroupRule(buildingName=None, groupId=None, ruleId=None):
    ...
    if request.method == 'POST':
        ...
        return render_template('ruleForm.html', rule=rule, insertionForGroup=True, availableTriggers=availableTriggers,
                               availableActions=availableActions)
...
def EPOCH(utcDatetimeString):
    ...
    utc_datetime = datetime.datetime.strptime(utcDatetimeString, '%b %d %Y %H:%M').replace(
