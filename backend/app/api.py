...
def register(username=None):
    if request.method == 'POST':
        ...
        try:
            return returnResult(
                usersManager.registerTemporary(newUserUsername=username, newUserPassword=password, newUserEmail=email,
                                               newUserPersonName=personName))
...
def storeFeedback(username=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(
                feedbackManager.storeFeedback(authorUuid=authorUuid, alternativeContact=alternativeContact, score=score,
                                              message=message))
        except Exception as e:
            return returnError(e)
...
def ruleCategories(username=None):
    if request.method == 'POST':
        ...
        try:
            ...
            #get the level of the user and filter the category
            userInfo = userManager.getInfo(username, userUuid)
            if userInfo["level"] != 100:
                categories.remove("DANGER")
...
def roomSimulation(username=None, buildingName=None, roomName=None):
    if request.method == 'POST':
        ...
        try:
            ...
            buildingsManager = BuildingsManager()
            buildingsManager.checkUserBinding(buildingName, username)
            roomSimulator = RoomSimulator(buildingName=buildingName, roomName=roomName,
                                          occupancyTimeRangeFrom=occupancyTimeRangeFrom,
                                          occupancyTimeRangeTo=occupancyTimeRangeTo, roomTemperature=roomTemperature,
                                          externalTemperature=externalTemperature, weather=weather)
...
def roomRules(username=None, buildingName=None, roomName=None):
    if request.method == 'POST':
        ...
        sessionKey = validateInput(request.form['sessionKey'])
        userUuid = validateInput(request.form['userUuid'])
        filterByAuthor = getBoolFromString(
            validateInput(request.form['filterByAuthor'])) if 'filterByAuthor' in request.form.keys() else False
        includeGroupsRules = getBoolFromString(
            validateInput(request.form['includeGroupsRules'])) if 'includeGroupsRules' in request.form.keys() else False
        orderByPriority = getBoolFromString(
            validateInput(request.form['orderByPriority'])) if 'orderByPriority' in request.form.keys() else False
        includeTriggerCategory = getBoolFromString(validateInput(
            request.form['includeTriggerCategory'])) if 'includeTriggerCategory' in request.form.keys() else False
        ...
        try:
            ...
            #get all the rules
            rules = roomsManager.getRules(roomName=roomName, buildingName=buildingName, username=usernameFilter,
                                          includeGroupsRules=includeGroupsRules, orderByPriority=orderByPriority,
                                          includeDisabled=True, categoriesFilter=categoriesFilter,
            ...
            userInfo = userManager.getInfo(username, userUuid)
            if userInfo["level"] != 100:
                flash(str(rules["rules"]))
                for rule in rules["rules"]:
                    flash(str(rule))
                    if rule["category"] == "DANGER":
                        remove.append(rule)
                for rule in remove:
                    flash(str(rule))
                    rules["rules"].remove(rule)


            return returnResult(rules)
...
def roomConflictingRules(username=None, buildingName=None, roomName=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(
                roomsManager.getConflictingRules(roomName=roomName, buildingName=buildingName, ruleBody=ruleBody))
        except Exception as e:
            return returnError(e)
...
def registerUser(username=None):
    if request.method == 'POST':
        ...
        try:
            ...
            userManager = UsersManager()
            return returnResult(
                userManager.register(creatorUuid, newUserUsername, newUserPassword, newUserEmail, newUserPersonName,
                                     newUserLevel))
        except Exception as e:
            return returnError(e)
...
def addGroupToBuilding(username=None, buildingName=None):
    if request.method == 'POST':
        ...
        userUuid = validateInput(request.form['userUuid'])
        description = validateInput(request.form['description'])
        crossRoomsValidation = validateInput(request.form[
            'crossRoomsValidation']) if 'crossRoomsValidation' in request.form.keys() else False  #BOOLEAN VALUE
        crossRoomsValidationCategories = request.form[
            'crossRoomsValidationCategories'] if 'crossRoomsValidationCategories' in request.form.keys() else None  #LIST IN JSON FORMAT
...
def editGroupInBuilding(username=None, buildingName=None, groupId=None):
    if request.method == 'POST':
        ...
        crossRoomsValidation = validateInput(request.form[
            'crossRoomsValidation']) if 'crossRoomsValidation' in request.form.keys() else False  #BOOLEAN VALUE
        crossRoomsValidationCategories = validateInput(request.form[
            'crossRoomsValidationCategories']) if 'crossRoomsValidationCategories' in request.form.keys() else None  #LIST IN JSON FORMAT
        ...
        try:
            ...
            return returnResult(
                buildingsManager.editGroup(groupId=groupId, buildingName=buildingName, description=description,
                                           crossRoomsValidation=crossRoomsValidation,
                                           crossRoomsValidationCategories=crossRoomsValidationCategories))
        except Exception as e:
            return returnError(e)
...
@api.route('/api/users/<username>/buildings/<buildingName>/rooms/<roomName>/rules/<ruleId>/setPriority',
           methods=['POST'])
def setRulePriority(username=None, buildingName=None, roomName=None, ruleId=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(
                roomsManager.setRulePriority(buildingName=buildingName, roomName=roomName, ruleId=ruleId,
...
def enableRuleInRoom(username=None, buildingName=None, roomName=None, ruleId=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(roomsManager.enableRule(ruleId=ruleId, buildingName=buildingName, roomName=roomName,
                                                        editorUuid=userUuid))
        except Exception as e:
            return returnError(e)
...
def addRuleToGroup(username=None, buildingName=None, groupId=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(groupsManager.addRule(priority=priority, buildingName=buildingName, groupId=groupId,
                                                      authorUuid=authorUuid, ruleBody=ruleBody))
        except Exception as e:
            return returnError(e)
...
def editRuleInGroup(username=None, buildingName=None, groupId=None, ruleId=None):
    if request.method == 'POST':
        ...
        try:
            ...
            return returnResult(
                groupsManager.editRule(ruleId=ruleId, priority=priority, buildingName=buildingName, groupId=groupId,
                                       authorUuid=authorUuid, ruleBody=ruleBody))
        except Exception as e:
            return returnError(e)
...
@api.route('/api/rule_optimizer/minimized_truth_table/<building>/<room>', methods=['GET'])
@cross_origin(origin='*',headers=['Content-Type','Authorization'])
def getMinimizedTable(building, room):
    if request.method == 'GET':
        # TODO: temporarily not singleton
        ruleOptimizer = RuleOptimizer(building, room)
        truthTable = ruleOptimizer.run()
