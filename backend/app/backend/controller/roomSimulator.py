...
class RoomSimulator:
    def __init__(self, buildingName=None, roomName=None, occupancyTimeRangeFrom=None, occupancyTimeRangeTo=None,
                 roomTemperature=None, externalTemperature=None, weather=None, currentDate=None):
    ...
    def start(self):
        ...
        for hour in range(0, 24):
            for minute in range(0, 60, 60):
                ...
                #TODO: self.weather per togliere l'hardcoding della weather
                ...
                for record in timeRecords:
                    if record[2] == target:
                        ...
                        if counter == 0:
                            ...
                            oldTime = float(record[I_TIME].replace(":","."))

                        time = float(record[I_TIME].replace(":","."))

                        if record[I_RULE_TEXT] != interval["ruleText"] or time>oldTime+1 or counter == (actionTargetsRecordsNumber[target] - 1):
                            if oldRecord:
                                interval["end"] = oldRecord[I_TIME]
                            else:
                                interval["end"] = record[I_TIME]
                            gantt[target].append(
                                {"from": interval["start"], "to": interval["end"], "status": interval["status"],
