...
from app.backend.controller.satisfactionManager import SatisfactionManager
...
class ActionExecutor:
    ...
    def solveConflicts(self, ruleList):
        ...
        for cat in categories:
            ...
            if len(rules) > 1:
                return self.conflictResolution(rules, cat)
            else:
                return rules[0]

    def conflictResolution(self, ruleList, category):
        flash('Executing CONFLICT RESOLUTION', 'red')

        if category in ['HVAC_HUM', 'HVAC_TEMP']:
            return self.satisfactionResolution(ruleList)
        else:
            return self.majorityWeightedVote(ruleList)

    def satisfactionResolution(self, ruleList):
        flash('Executing SATISFACTION RESOLUTION', 'red')
        print 'on rules: ', ruleList
        ...
        for r in ruleList:
            ...
            values += priority*satisfactionManager.agauss(mean,sigma1,sigma2)

        finalValue = satisfactionManager.getMax(values)
        finalPriority = satisfactionManager.getMaxPriority(values)


        commonRule = ruleList[0].getDict()
        resultingRule = Rule(id='SYNTHETIZED', priority=finalPriority, category=commonRule['category'],
        ...
        print resultingRule
        print finalValue, finalPriority

        return resultingRule


    def majorityWeightedVote(self, ruleList):
        flash('Executing MAJORITY VOTE RESOLUTION', 'red')
        print 'on rules: ', ruleList
        ...
        for r in ruleList:
            r = r.getDict()
            if r['consequent'] not in dict.keys():
                dict[r['consequent']] = r['priority']
            else:
                dict[r['consequent']] += r['priority']
        ...
        import operator
        winningConsequent = max(dict.iteritems(), key=operator.itemgetter(1))[0]


        commonRule = ruleList[0].getDict()
        resultingRule = Rule(id='SYNTHETIZED', priority=dict[winningConsequent], category=commonRule['category'],
                             buildingName=commonRule['buildingName'], groupId=commonRule['groupId'],
                             roomName=commonRule['roomName'], authorUuid='SYNTHESIZED',
                             antecedent='SYNTHESIZED', consequent=winningConsequent,
                             enabled=True, deleted=False)

        print 'winningRule', resultingRule
